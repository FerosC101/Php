...
<head>
    ...
    <title>' . NAME . ' - CV</title>
    <style>
        body { font-family: Arial, Helvetica, sans-serif; margin: 0; padding: 40px; background: #fff; color: #000; line-height: 1.6; }
        .container { max-width: 800px; margin: auto; }
        .header { text-align: center; margin-bottom: 20px; }
        h1 { font-size: 28px; margin-bottom: 5px; }
        .contact { font-size: 14px; margin-bottom: 20px; }
        h2 { font-size: 20px; border-bottom: 2px solid #000; padding-bottom: 4px; margin-top: 25px; }
        ul { margin: 10px 0 10px 20px; }
        li { margin-bottom: 6px; }
        .section { margin-bottom: 15px; }
        .project-title { font-weight: bold; }
        .edu, .exp-date { font-style: italic; font-size: 14px; margin-bottom: 5px; }
        .download-btn { display: inline-block; padding: 10px 15px; background: #007BFF; color: #fff; text-decoration: none; border-radius: 5px; font-size: 14px; }
...
<body>
    <div class="container">
        ...
        <ul>';
            ...
            echo "<li><strong>$category:</strong> $value</li>";
            }
    echo '   </ul>
        </div>
        ...
        <h2>EXPERIENCE</h2>';
            ...
            echo "<p><strong>{$exp['role']}</strong> <span class='exp-date'>{$exp['date']}</span></p><ul>";
    ...
    echo ' </div>
        ...
        <h2>PROJECTS</h2>';
            ...
            echo "<p class='project-title'>{$proj['title']} <span class='exp-date'>{$proj['date']}</span></p><ul>";
    ...
    echo ' </div>
        ...
        <h2>ACHIEVEMENTS</h2>
        <ul>';
            foreach ($achievements as $category => $value) {
            echo "<li><strong>$category:</strong> $value</li>";
            }
    echo '   </ul>
        </div>
